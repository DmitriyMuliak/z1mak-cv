...
const client = new GoogleGenAI({ apiKey: publicPrEnv.NEXT_PUBLIC_GEMINI_API_KEY });
...
type BuildPromptSettingsOptions = { mode: Mode; locale: string };
...
const getImmediateInstruction = (mode: Mode, locale: string) => {
  ...
  // 1. Common Basics
  lines.push(builder.add(`Adhere to the rules and persona described in the system prompt.`));
  lines.push(builder.add(`Analyze the input data and populate the output JSON structure.`));
  lines.push(
    builder.add(
  ...
  if (evaluationMode === 'byJob') {
    ...
    // General Mode
    lines.push(
      builder.add(`Evaluate the candidate based on the implied role title found in the CV header.`),
    );
    lines.push(builder.add(`Focus on "Actionable Improvement Plan" to make this CV market-ready.`));
  }
  ...
  if (isHardMode) {
    lines.push(
      builder.add(`Provide a detailed "detailedSkillAnalysis" parsing every skill mentioned.`),
    );
    lines.push(
      builder.add(`Generate "suggestedInterviewQuestions" to probe the identified weak spots.`),
    );
  } else {
    lines.push(builder.add(`Provide a high-level summary without deep granular skill breakdown.`));
  }
