...
import { analyzeResume, AnalyzePayload, ResumeErrorCode } from '@/actions/resume/resumeActions';
...
interface SendToAnalyzeStreamActionState {
...
export const sendToAnalyzeStreamAction = async (
  data: SendToAnalyzeFEType,
  stateOptional?: SendToAnalyzeStreamActionState,
): Promise<ResultReturn<AnalyzeSuccessData>> => {
  const state = stateOptional as SendToAnalyzeStreamActionState;
...
const extractContent = async (
  ...
  if (method === 'file') {
    ...
    try {
      const { parseFile } = await import('../parsers/parseFile');
